...
public class ExecuteProgramController {
    ...
    @FXML
    private TableView<Pair<String, String>> procedureTable;

    @FXML
    private TableColumn<Pair<String, String>, String> nameColumnProcedureTable;

    @FXML
    private TableColumn<Pair<String, String>, String> bodyColumnProcedureTable;
    ...
    public void initialize() {
        ...
        // set TextField noOfPrograms to be uneditable
        this.noOfPrograms.setEditable(false);
        ...
        // set the cell factory for the procedure table
        this.nameColumnProcedureTable.setCellValueFactory(param -> new SimpleStringProperty(param.getValue().getKey()));
        this.bodyColumnProcedureTable.setCellValueFactory(param -> new SimpleStringProperty(param.getValue().getValue()));
    }
    public void setStatement(StatementInterface newStatement, MyProcedureTableInterface procedureTableInterface , String logFilePath) {
        ProgramState programState = new ProgramState(new MyStack<>(), new MyDictionary<>(), new MyList<>(),
                new MyDictionary<>(), new MyHeap<>(), procedureTableInterface, newStatement);
        try {
    ...
    public void populateProcedureTable(){
        MyProcedureTableInterface procedureTable = this.controller.getProcedureTable();

        // create a list of entries for the procedure table
        ArrayList<Pair<String, String>> procedureTableEntries = new ArrayList<>();
        for (Map.Entry<String, Pair<List<String>, StatementInterface>> entry : procedureTable.getContent().entrySet()) {
            String name = entry.getKey() + "(" + entry.getValue().getKey().toString() + ")";
            String body = entry.getValue().getValue().toString();
            procedureTableEntries.add(new Pair<String, String>(name, body));
        }

        this.procedureTable.getItems().setAll(FXCollections.observableArrayList(procedureTableEntries));
