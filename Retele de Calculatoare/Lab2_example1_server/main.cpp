...
void time_out(int semnal) {
    ...
    r = htonl(r);
    printf("Time out.\n");
    send(c, (char*) &r, sizeof(int32_t), 0);
...
int main() {
    // loading the dll
    {
        ...
        if (result != 0) {
            fprintf(stderr, "Eroare la initializarea WinSock.\n");
            return -1;
        }
    }
    ...
    int returned = bind(serverSocket, (struct sockaddr *) &server, sizeof(struct sockaddr_in));
    if (returned < 0) {
    ...
    while (1) { // deserveste oricati clienti
        ...
        SOCKET
        c = accept(s, (struct sockaddr *) &client, &length);
